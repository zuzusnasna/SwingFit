...
> 골프 스윙 영상을 AI로 분석하여 골프공의 비행 데이터를 추정하는 Android 애플리케이션

한신대학교 **캡스톤디자인 팀 프로젝트**로 진행한 Android 기반 골프 스윙 분석 애플리케이션입니다.

스마트폰으로 촬영한 골프 스윙 영상을 입력하면 TensorFlow Lite 기반 객체 탐지 모델을 이용하여 골프공을 검출하고, 프레임별 위치를 추적하여 **공의 이동 궤적, 임팩트 시점, 초기 속도, 발사각, 예상 캐리 거리**를 분석합니다.

## 📌 프로젝트 소개

골프 스윙을 분석하기 위해서는 전문 장비나 골프 시뮬레이터가 필요한 경우가 많습니다.

**SwingFit**은 이러한 분석 과정을 스마트폰 영상과 AI 기술을 활용하여 구현하는 것을 목표로 제작했습니다.

사용자가 촬영한 골프 스윙 영상을 선택하고 공이 위치한 영역(ROI)을 지정하면, 앱에서 영상 프레임을 분석하여 골프공의 위치를 추적합니다.

이후 추적 데이터를 기반으로 임팩트 시점을 탐색하고 공의 비행 데이터를 계산하여 영상 위에 시각적으로 표시합니다.

## 🎯 주요 기능

### 1. 골프 스윙 영상 선택

- Android 파일 선택기를 통한 영상 선택
- Media3 ExoPlayer를 이용한 영상 재생
- 영상 분석과 재생 화면을 하나의 UI에서 제공

### 2. ROI(Region of Interest) 설정

영상 전체가 아닌 골프공이 위치할 영역을 사용자가 직접 지정할 수 있습니다.

- 화면 드래그를 통한 ROI 설정
- 정규화 좌표 기반 ROI 관리
- 불필요한 영역의 분석을 줄여 객체 탐지 효율 향상

### 3. AI 기반 골프공 검출

TensorFlow Lite 모델을 Android 기기에서 직접 실행하여 영상 프레임에서 골프공을 탐지합니다.

- `ball-fp16.tflite`
- `ball-int8.tflite`
- 640 × 640 입력 기반 분석
- Detection Confidence를 이용한 공 후보 판별
- GPU Delegate 및 CPU fallback 지원

### 4. 골프공 위치 추적

각 프레임에서 검출된 골프공의 위치를 저장하여 이동 경로를 구성합니다.

검출 결과에는 다음 정보가 포함됩니다.
...
넓은 시간 간격으로 후보 시점을 탐색한 뒤 해당 구간을 다시 세밀하게 탐색하여 임팩트 시점을 보정합니다.
...
공의 검출 및 추적 데이터를 이용하여 다음 정보를 계산합니다.
...
Android Canvas 기반 `OverlayView`를 사용하여 분석 결과를 영상 위에 표시합니다.
